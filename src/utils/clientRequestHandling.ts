...
function handleClientRequest(
  ...
  console.log(
    `${getServerMessageDatedTitle()} [wsId: ${
      ws.id
    }] Received message: ${messageStr}`
  );
  ...
  try {
    ...
    console.log(
      `${getServerMessageDatedTitle()} [wsId: ${
        ws.id
      }] Response with message: ${failMessage}`
    );
  ...
  switch (clientMessage.type) {
    case "reg": {
      ...
      console.log(
        `${getServerMessageDatedTitle()} [wsId: ${
          ws.id
        }] Response with message: ${responseStr}`
      );
    ...
    case "create_room": {
      ...
      sentToAllWs(responseStr, (socket, socketMessage) => {
        console.log(
          `${getServerMessageDatedTitle()} [wsId: ${
            socket.id
          }] Response with message: ${socketMessage}`
        );
      });
    ...
    case "add_user_to_room": {
      const addToRoomResponse = getAddToRoomResponseMessage(
        clientMessage,
        ws.id
      );
    ...
    case "randomAttack": {
      const randomAttackResponse =
        getRandomAttackResponseMessage(clientMessage);

      if (randomAttackResponse) {
        sentToWsById(randomAttackResponse, (socket, socketMessage) => {
          console.log(
            `${getServerMessageDatedTitle()} [wsId: ${
              socket.id
            }] Response with message: ${socketMessage}`
    ...
    case "single_play": {
      console.log(
        `${getServerMessageDatedTitle()} Bot not ready for game (not implemented)!`
      );
      return;
    }
    default: {
